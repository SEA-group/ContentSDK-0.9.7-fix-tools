...
## How to use
1. Put `LOD_transporter.m`, `Obsolete_node_remover.m` and `content_0.10.5.0.mat` in `[WoWS folder]/bin/[largest number]/res_mods/PnFMods/`
...
**[Download](https://github.com/SEA-group/ContentSDK-0.9.7-fix-tools/releases/download/0.10.5.0a/ContentSDK_fix_0.10.5.0a.zip)**
...
2. Open cmd or powershell in the same folder, type `wowsunpack.exe -l -x bin\[build_number]\idx -p ..\..\..\res_packages -I content/gameplay/*/misc/* -I content/gameplay/*/technical/* -X *textures* -X *lod* > content.txt`; note that the build number changes according to game client version;
3. Put `content.txt` beside `Available_MP_nodes_check.m`, **check the version number in line 19**, and run the latter. The path of this script does not matter.
